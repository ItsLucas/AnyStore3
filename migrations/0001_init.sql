...
--
-- Logical schema is normative; see AnyStore_Architecture_v1.md section 7.
...
-- Uniqueness applies to live objects only, so a deleted name can be reused.
...
-- The fixed root folder.
INSERT INTO objects (id, revision, kind, name, parent_id, metadata, created_at, updated_at)
...
-- Supports the maintenance sweep for abandoned sessions.
CREATE INDEX uploads_expiry_idx ON uploads(expires_at)
WHERE state IN ('initiating', 'ready', 'completing');
...
-- An explicit sequence (rather than an identity column) lets a transaction
-- reserve `seq` before insert, so `change_id` can be derived from it and stay
-- monotonic in Change order.
...
CREATE TABLE changes (
    ...
    action              TEXT NOT NULL CHECK (action IN (
                            'created', 'metadata_updated', 'renamed', 'moved',
                            'content_ready', 'content_replaced', 'deleted')),
...
-- Singleton watermark. Retained so that a cursor pointing before the purged
-- history can still be rejected with `changes_cursor_expired` after the rows
-- themselves are gone.
